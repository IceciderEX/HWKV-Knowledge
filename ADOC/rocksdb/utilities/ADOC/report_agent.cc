#include "include/rocksdb/utilities/adoc_tuner.h"
#include "include/rocksdb/utilities/report_agent.h"
...
// 在基类报告的 secs_elapsed 和 interval_qps 基础上
// 额外增加了当前的 batchsize 和后台线程数
Status ReporterAgent::ReportLine(int secs_elapsed, int total_ops_done_snapshot) {
...
void ReporterAgentWithTuning::DetectAndTuning(int secs_elapsed) {
  // report 线程在 SleepAndReport 循环中被调用
  // 达到时间间隔，
...
void ReporterAgentWithTuning::ApplyChangePointsInstantly(std::vector<ChangePoint>* points) {
  std::unordered_map<std::string, std::string>* new_cf_options;
  std::unordered_map<std::string, std::string>* new_db_options;
  // 将数据库级别和列族级别的配置项分别放入两个不同的 map 中
  ...
  for (auto point : *points) {
    if (point.is_db_level) {
      new_db_options->emplace(point.option, point.value);
    } else {
      new_cf_options->emplace(point.option, point.value);
    }
  }
  points->clear();
  Status s;
  // 创建新的独立线程去调用 update_db_options 和 update_cf_options 函数来应用这些 map
...
void ReporterAgentWithTuning::PopChangePoints(int secs_elapsed) {
  // 检查如果某个 ChangePoint 的预定执行时间 (change_timing) 已经到达
  // 就应用它
