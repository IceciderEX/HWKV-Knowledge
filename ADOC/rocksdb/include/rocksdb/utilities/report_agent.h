...
class ReporterAgent {
 ...
 public:
  ...
  // 初始化所有成员变量，打开报告文件并写入 header_string_，创建并启动 reporting_thread_ 后台线程 
 ...
 protected:
  ...
  // 对操作系统的抽象层，提供了文件操作、线程管理、获取当前时间等功能
  Env* env_;
  // 管理报告文件的 vector
  std::unique_ptr<WritableFile> report_file_;
  // 记录自启动以来完成的总操作数
  std::atomic<int64_t> total_ops_done_;
  // 上一次报告 total_ops_done_ 的快照值
  ...
  void SleepAndReport() {
    ...
    while (true) {
      ...
      // 获取当前总操作数的快照，并计算总运行时间 secs_elapsed
...
class ReporterAgentWithTuning : public ReporterAgent {
 ...
 public:
  // Recorded history lsm shape, here we record 10 secs
  ...
  // 先调用 tuner 收集数据并给出指令（changepoints），之后 apply change points
